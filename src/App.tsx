...
import { calculateBonusTax } from './logic/bonusTaxCalculations';
import type { BonusTaxInput, FilingStatus } from './logic/bonusTaxCalculations';

function App() {
  const [values, setValues] = useState<BonusTaxInput>({
    bonusAmount: 5000,
    annualSalary: 75000,
    filingStatus: 'single',
    flatTaxRate: 22
  ...
  const handleChange = (field: keyof BonusTaxInput, value: number | boolean | FilingStatus) => {
    setValues(prev => ({ ...prev, [field]: value }));
  };

  const result = calculateBonusTax(values);
